...
contract UniswapExchange is ERC20 {
 ...
 /**
  ...
  function getInputPrice(uint256 input_amount, uint256 input_reserve, uint256 output_reserve) public view returns (uint256) {
    require(input_reserve > 0 && output_reserve > 0, "invalid reserve values");
 ...
 /**
  ...
  function ethToTokenInput(uint256 eth_sold, uint256 min_tokens, uint256 deadline, address buyer, address recipient) private returns (uint256) {
    require(deadline >= block.timestamp && eth_sold > 0 && min_tokens > 0, "invalid deadline, eth_sold can't and/or min_tokens");
    uint256 token_reserve = token.balanceOf(address(this));
    uint256 tokens_bought = getInputPrice(eth_sold, address(this).balance.sub(eth_sold), token_reserve);
    require(tokens_bought >= min_tokens, "invalid min_tokens");
  ...
  function ethToTokenOutput(uint256 tokens_bought, uint256 max_eth, uint256 deadline, address payable buyer, address recipient) private returns (uint256) {
    require(deadline >= block.timestamp && tokens_bought > 0 && max_eth > 0, "invalid deadline, tokens_bought and/or max_eth");
    uint256 token_reserve = token.balanceOf(address(this));
  ...
  function tokenToEthInput(uint256 tokens_sold, uint256 min_eth, uint256 deadline, address buyer, address payable recipient) private returns (uint256) {
    require(deadline >= block.timestamp && tokens_sold > 0 && min_eth > 0, "invalid deadline, tokens_sold and/or min_eth");
    uint256 token_reserve = token.balanceOf(address(this));
    uint256 eth_bought = getInputPrice(tokens_sold, token_reserve, address(this).balance);
    uint256 wei_bought = eth_bought;
    require(wei_bought >= min_eth, "invalid min_eth");
  ...
  function tokenToEthOutput(uint256 eth_bought, uint256 max_tokens, uint256 deadline, address buyer, address payable recipient) private returns (uint256) {
    require(deadline >= block.timestamp && eth_bought > 0, "invalid deadline and/or eth_bought");
    uint256 token_reserve = token.balanceOf(address(this));
    uint256 tokens_sold = getOutputPrice(eth_bought, token_reserve, address(this).balance);
    // tokens sold is always > 0
    require(max_tokens >= tokens_sold, "invalid max_tokens");
  ...
  {
    require(deadline >= block.timestamp && tokens_sold > 0 && min_tokens_bought > 0 && min_eth_bought > 0, "invalid deadline, tokens_sold, min_tokens_bought and/or min_eth_bought");
    require(exchange_addr != address(this) && exchange_addr != address(0), "invalid exchange_address");
    uint256 token_reserve = token.balanceOf(address(this));
    uint256 eth_bought = getInputPrice(tokens_sold, token_reserve, address(this).balance);
    uint256 wei_bought = eth_bought;
    require(wei_bought >= min_eth_bought, "invalid min_eth_bought");
    SafeERC20.transferFrom(address(token), buyer, address(this), tokens_sold);
  ...
  function tokenToTokenOutput(
    ...
    private returns (uint256) 
  {
    require(deadline >= block.timestamp && (tokens_bought > 0 && max_eth_sold > 0), "invalid dadline, tokens_bought and/or max_eth_sold");
    require(exchange_addr != address(this) && exchange_addr != address(0), "invalid exchange_addr");
    uint256 eth_bought = IUniswapExchange(exchange_addr).getEthToTokenOutputPrice(tokens_bought);
    uint256 token_reserve = token.balanceOf(address(this));
    uint256 tokens_sold = getOutputPrice(eth_bought, token_reserve, address(this).balance);
    // tokens sold is always > 0
    require(max_tokens_sold >= tokens_sold && max_eth_sold >= eth_bought, "invalid max_tokens_sold and/or max_eth_sold");
    SafeERC20.transferFrom(address(token), buyer, address(this), tokens_sold);
  ...
  function getEthToTokenInputPrice(uint256 eth_sold) public view returns (uint256) {
    require(eth_sold > 0, "invalid eth_sold");
    uint256 token_reserve = token.balanceOf(address(this));
  ...
  function getEthToTokenOutputPrice(uint256 tokens_bought) public view returns (uint256) {
    require(tokens_bought > 0, "invalid tokens_bought");
    uint256 token_reserve = token.balanceOf(address(this));
  ...
  function getTokenToEthInputPrice(uint256 tokens_sold) public view returns (uint256) {
    require(tokens_sold > 0, "invalid tokens_sold");
    uint256 token_reserve = token.balanceOf(address(this));
    uint256 eth_bought = getInputPrice(tokens_sold, token_reserve, address(this).balance);
    return eth_bought;
  }
  ...
  function getTokenToEthOutputPrice(uint256 eth_bought) public view returns (uint256) {
    require(eth_bought > 0, "invalid eth_bought");
    uint256 token_reserve = token.balanceOf(address(this));
  ...
  function addLiquidity(uint256 min_liquidity, uint256 max_tokens, uint256 deadline) public payable returns (uint256) {
    require(deadline > block.timestamp && max_tokens > 0 && msg.value > 0, "invalid deadline, max_tokens and/or msg.value");
    uint256 total_liquidity = _totalSupply;

    if (total_liquidity > 0) {
      require(min_liquidity > 0, "invalid min_liquidity");
      uint256 eth_reserve = address(this).balance.sub(msg.value);
      uint256 token_reserve = token.balanceOf(address(this));
      uint256 token_amount = (msg.value.mul(token_reserve) / eth_reserve).add(1);
      uint256 liquidity_minted = msg.value.mul(total_liquidity) / eth_reserve;
      require(max_tokens >= token_amount && liquidity_minted >= min_liquidity, "invalid max_tokens and/or min_liquidity");
      ...
      require(address(factory) != address(0) && address(token) != address(0) && msg.value >= 1000000000, "invalid factory, token and/or msg.value");
      require(factory.getExchange(address(token)) == address(this), "invalid token exchange");
  ...
  function removeLiquidity(uint256 amount, uint256 min_eth, uint256 min_tokens, uint256 deadline) public returns (uint256 out, uint256) {
    require(amount > 0 && deadline > block.timestamp && min_eth > 0 && min_tokens > 0, "invalid amount, deadline, min_eth and/or min_tokens");
    uint256 total_liquidity = _totalSupply;
    require(total_liquidity > 0, "invalid total_liquidity");
    uint256 token_reserve = token.balanceOf(address(this));
    ...
    require(eth_amount >= min_eth && token_amount >= min_tokens, "invalid min_eth and/or min_tokens");
